...
async def download_file(session, url, directory, semaphore, progress_callback, async_mode=True, start_time=None):
...
async def handle_async_downloads(session, base_url_location_eia, file_types, full_path, progress_callback, random_delay_r5y):
    ...
    try:
        ...
        tasks = [asyncio.create_task(handle_download(session, urljoin(base_url_location_eia, href['href']), full_path, download_semaphore, progress_callback, get_random_delay(random_delay_r5y)))
